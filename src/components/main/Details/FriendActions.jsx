        ...
        <div>
            <div className='flex flex-col justify-center items-start gap-4 border border-gray-300 rounded-xl p-6 shadow-sm'>
                ...
                <div className='flex justify-between items-center w-full'>
                    <button className='btn btn-outline text-center px-4' onClick={() => { handleFriendsActions("call", friend, new Date(Date.now()).toISOString()) }}>
                        <p className='font-bold text-2xl'><MdAddCall /></p>
                        <p className='text-sm text-gray-500'>Call</p>
                    </button>
                    <button className='btn btn-outline text-center px-4' onClick={() => { handleFriendsActions("video", friend, new Date(Date.now()).toISOString()) }}>
                        <p className='font-bold text-2xl'><FaVideo /></p>
                        <p className='text-sm text-gray-500'>Video</p>
                    </button>
                    <button className='btn btn-outline text-center px-4' onClick={() => { handleFriendsActions("text", friend, new Date(Date.now()).toISOString()) }}>
